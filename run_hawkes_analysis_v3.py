#!/usr/bin/env python3
"""
V3: Script to run Hawkes analysis on bund.npz data using hawkes_conditional_law_v3.py
...
def main():
    # Load the bund.npz file
    print("V3: Loading bund.npz file...")
    data = np.load('bund.npz', allow_pickle=True).items()
    ...
    print(f"V3: Kernel discretization: {len(kernel_discretization)} points from {kernel_discretization[0]} to {kernel_discretization[-1]}")
    
    # Create and configure Hawkes learner V3 with multiprocessing
    print("V3: Creating HawkesConditionalLaw learner with multiprocessing for PPCL...")
    hawkes_learner = HawkesConditionalLaw(
        ...
        min_support=1e-4, 
        max_support=1, 
        n_threads=4  # Enable multiprocessing
    )
    
    # Fit the model
    print("V3: Fitting Hawkes model...")
    hawkes_learner.fit(events)
    
    print("V3: Model fitting completed!")
    print(f"V3: Number of nodes: {hawkes_learner.n_nodes}")
    print(f"V3: Baseline intensities: {hawkes_learner.baseline}")
    
    # Numerical verification outputs
    print("V3: Computing numeric verification outputs...")
    kernels_norms = hawkes_learner.get_kernel_norms()
    eigvals = np.linalg.eigvals(kernels_norms)
    spectral_radius = np.max(np.abs(eigvals))
    print("V3: Kernels norms matrix:\n", kernels_norms)
    print("V3: Eigenvalues of kernels norms:", eigvals)
    print(f"V3: Spectral radius (branching ratio): {spectral_radius:.6f}")
    print(f"V3: Mean intensities: {hawkes_learner.mean_intensity}")
    
    # Plot the Hawkes kernel norms
    print("V3: Plotting Hawkes kernel norms...")
    fig = plot_hawkes_kernel_norms(hawkes_learner,
                                   node_names=["P_u", "P_d", "T_a", "T_b"],
                                   show=False)
    
    # Save the plot with V3 suffix
    plot_filename = "hawkes_kernel_norms_v3.png"
    fig.savefig(plot_filename, dpi=300, bbox_inches='tight')
    print(f"V3: Plot saved as: {plot_filename}")
    

    print("V3: Analysis completed successfully!")
