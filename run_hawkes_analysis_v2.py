#!/usr/bin/env python3
"""
V2: Script to run Hawkes analysis on bund.npz data using hawkes_conditional_law_v2.py
...
def main():
    ...
    print("V2: Loading bund.npz file...")
    ...
    print(f"V2: Kernel discretization: {len(kernel_discretization)} points from {kernel_discretization[0]} to {kernel_discretization[-1]}")
    
    # Create and configure Hawkes learner V2
    print("V2: Creating HawkesConditionalLaw learner with numba optimizations...")
    ...
    # Fit the model
    print("V2: Fitting Hawkes model...")
    hawkes_learner.fit(events)
    
    print("V2: Model fitting completed!")
    print(f"V2: Number of nodes: {hawkes_learner.n_nodes}")
    print(f"V2: Baseline intensities: {hawkes_learner.baseline}")
    
    # Numerical verification outputs
    print("V2: Computing numeric verification outputs...")
    kernels_norms = hawkes_learner.get_kernel_norms()
    eigvals = np.linalg.eigvals(kernels_norms)
    spectral_radius = np.max(np.abs(eigvals))
    print("V2: Kernels norms matrix:\n", kernels_norms)
    print("V2: Eigenvalues of kernels norms:", eigvals)
    print(f"V2: Spectral radius (branching ratio): {spectral_radius:.6f}")
    print(f"V2: Mean intensities: {hawkes_learner.mean_intensity}")
    
    # Plot the Hawkes kernel norms
    print("V2: Plotting Hawkes kernel norms...")
    fig = plot_hawkes_kernel_norms(hawkes_learner,
                                   node_names=["P_u", "P_d", "T_a", "T_b"],
                                   show=False)
    
    # Save the plot with V2 suffix
    plot_filename = "hawkes_kernel_norms_v2.png"
    fig.savefig(plot_filename, dpi=300, bbox_inches='tight')
    print(f"V2: Plot saved as: {plot_filename}")
