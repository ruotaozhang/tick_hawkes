#!/usr/bin/env python3
"""
Script to run Hawkes analysis on bund2.npz (enlarged dataset) using hawkes_conditional_law.py
"""
...
def main():
    ...
    print("BUND2: Loading bund2.npz file (enlarged dataset)...")
    data = np.load('bund2.npz', allow_pickle=True).items()
    
    events = [list(timestamps) for _, timestamps in data]

    print(f"BUND2: Loaded {len(events)} time series with enlarged dataset")

    # Define kernel discretization
    kernel_discretization = np.hstack((0, np.logspace(-5, 0, 50)))
    print(f"BUND2: Kernel discretization: {len(kernel_discretization)} points from {kernel_discretization[0]} to {kernel_discretization[-1]}")
    
    # Create and configure Hawkes learner
    print("BUND2: Creating HawkesConditionalLaw learner...")
    ...
    # Fit the model
    print("BUND2: Fitting Hawkes model on enlarged dataset...")
    hawkes_learner.fit(events)
    
    print("BUND2: Model fitting completed!")
    print(f"BUND2: Number of nodes: {hawkes_learner.n_nodes}")
    print(f"BUND2: Baseline intensities: {hawkes_learner.baseline}")
    
    # Numerical verification outputs - focus on kernel norm matrix
    print("BUND2-V1: Computing kernel norm matrix for comparison...")
    kernels_norms = hawkes_learner.get_kernel_norms()
    eigvals = np.linalg.eigvals(kernels_norms)
    spectral_radius = np.max(np.abs(eigvals))
    
    print(f"BUND2-V1: Matrix shape: {kernels_norms.shape}")
    print(f"BUND2-V1: Matrix min/max: {kernels_norms.min():.6f} / {kernels_norms.max():.6f}")
    print(f"BUND2-V1: Matrix norm (Frobenius): {np.linalg.norm(kernels_norms, 'fro'):.6f}")
    print(f"BUND2-V1: Spectral radius: {spectral_radius:.6f}")
    print("BUND2-V1: First 5x5 submatrix:")
    print(kernels_norms[:5, :5])
    print("BUND2-V1: Diagonal elements:")
    print(np.diag(kernels_norms))
    
    # Save matrix for comparison
    np.save('kernels_norms_v1.npy', kernels_norms)
    print("BUND2-V1: Kernel matrix saved as kernels_norms_v1.npy")
    
    print("BUND2-V1: Analysis completed successfully!")
