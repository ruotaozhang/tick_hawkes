#!/usr/bin/env python3
"""
Script to run Hawkes analysis on bund.npz data using hawkes_conditional_law.py
"""
...
def main():
    ...
    print(f"Kernel discretization: {len(kernel_discretization)} points from {kernel_discretization[0]} to {kernel_discretization[-1]}")
    
    # Create and configure Hawkes learner
    print("Creating HawkesConditionalLaw learner...")
    ...
    # Fit the model
    print("Fitting Hawkes model...")
    hawkes_learner.fit(events)
    
    print("Model fitting completed!")
    print(f"Number of nodes: {hawkes_learner.n_nodes}")
    print(f"Baseline intensities: {hawkes_learner.baseline}")
    
    # Numerical verification outputs
    print("Computing numeric verification outputs...")
    kernels_norms = hawkes_learner.get_kernel_norms()
    eigvals = np.linalg.eigvals(kernels_norms)
    spectral_radius = np.max(np.abs(eigvals))
    print("Kernels norms matrix:\n", kernels_norms)
    print("Eigenvalues of kernels norms:", eigvals)
    print(f"Spectral radius (branching ratio): {spectral_radius:.6f}")
    print(f"Mean intensities: {hawkes_learner.mean_intensity}")
    
    # Plot the Hawkes kernel norms
    print("Plotting Hawkes kernel norms...")
    fig = plot_hawkes_kernel_norms(hawkes_learner,
                                   node_names=["P_u", "P_d", "T_a", "T_b"],
                                   show=False)
    
    # Save the plot
    plot_filename = "hawkes_kernel_norms.png"
