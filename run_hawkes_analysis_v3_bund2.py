#!/usr/bin/env python3
"""
V3: Script to run Hawkes analysis on bund2.npz (enlarged dataset) using hawkes_conditional_law_v3.py
Adds multiprocessing for _PointProcessCondLaw within incremental_fit for speed.
"""

import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1 import make_axes_locatable
from hawkes_conditional_law_v3 import HawkesConditionalLaw
...
def main():
    ...
    print("V3-BUND2: Loading bund2.npz file (enlarged dataset)...")
    data = np.load('bund2.npz', allow_pickle=True).items()
    
    events = [list(timestamps) for _, timestamps in data]

    print(f"V3-BUND2: Loaded {len(events)} time series with enlarged dataset")

    # Define kernel discretization
    kernel_discretization = np.hstack((0, np.logspace(-5, 0, 50)))
    print(f"V3-BUND2: Kernel discretization: {len(kernel_discretization)} points from {kernel_discretization[0]} to {kernel_discretization[-1]}")
    
    # Create and configure Hawkes learner V3 with multiprocessing
    print("V3-BUND2: Creating HawkesConditionalLaw learner with multiprocessing for PPCL...")
    hawkes_learner = HawkesConditionalLaw(
        ...
        min_support=1e-4, 
        max_support=1, 
        n_threads=-1  # Enable multiprocessing
    )
    
    # Fit the model
    print("V3-BUND2: Fitting Hawkes model on enlarged dataset...")
    hawkes_learner.fit(events)
    
    print("V3-BUND2: Model fitting completed!")
    print(f"V3-BUND2: Number of nodes: {hawkes_learner.n_nodes}")
    print(f"V3-BUND2: Baseline intensities: {hawkes_learner.baseline}")
    
    # Numerical verification outputs - focus on kernel norm matrix
    print("BUND2-V3: Computing kernel norm matrix for comparison...")
    kernels_norms = hawkes_learner.get_kernel_norms()
    eigvals = np.linalg.eigvals(kernels_norms)
    spectral_radius = np.max(np.abs(eigvals))
    
    print(f"BUND2-V3: Matrix shape: {kernels_norms.shape}")
    print(f"BUND2-V3: Matrix min/max: {kernels_norms.min():.6f} / {kernels_norms.max():.6f}")
    print(f"BUND2-V3: Matrix norm (Frobenius): {np.linalg.norm(kernels_norms, 'fro'):.6f}")
    print(f"BUND2-V3: Spectral radius: {spectral_radius:.6f}")
    print("BUND2-V3: First 5x5 submatrix:")
    print(kernels_norms[:5, :5])
    print("BUND2-V3: Diagonal elements:")
    print(np.diag(kernels_norms))
    
    # Save matrix for comparison
    np.save('kernels_norms_v3.npy', kernels_norms)
    print("BUND2-V3: Kernel matrix saved as kernels_norms_v3.npy")
    
    print("BUND2-V3: Analysis completed successfully!")
