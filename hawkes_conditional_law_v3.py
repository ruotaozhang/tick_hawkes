...
class Base:
    """Minimal replacement for tick.base.Base"""
    def __init__(self):
        pass
...
class ThreadPool:
    """Minimal replacement for tick.base.ThreadPool (kept for API parity)."""
    def __init__(self, with_lock=True, max_threads=1):
        self.max_threads = max_threads
        self.lock = Lock() if with_lock else None
        self.work_items = []
    
    def add_work(self, func, *args):
        """Add work to be executed"""
        self.work_items.append((func, args))
    
    def start(self):
        """Execute all work items sequentially (placeholder)."""
        for func, args in self.work_items:
            func(*args)
        self.work_items = []
...
@njit(cache=True)
def _numba_compute_mark_stats(marks, marked_intervals, mark_min_current, mark_max_current):
    """Numba-accelerated mark statistics computation."""
    n_events = len(marks)
    n_intervals = len(marked_intervals)
    
    if n_events == 0:
        return np.zeros(n_intervals, dtype=np.int64), mark_min_current, mark_max_current
    
    # Compute mark increments
    mark_increments = np.empty_like(marks)
    mark_increments[0] = marks[0]
    for i in range(1, n_events):
        mark_increments[i] = marks[i] - marks[i-1]
    
    # Update min/max
    mark_min_new = min(mark_min_current, np.min(mark_increments))
    mark_max_new = max(mark_max_current, np.max(mark_increments))
    
    # Count events in each interval
    interval_counts = np.zeros(n_intervals, dtype=np.int64)
    for i in range(n_events):
        mark_val = mark_increments[i]
        for j in range(n_intervals):
            if mark_val >= marked_intervals[j][0] and mark_val < marked_intervals[j][1]:
                interval_counts[j] += 1
    
    return interval_counts, mark_min_new, mark_max_new


@njit(cache=True)
def _numba_count_valid_events(timestamps, T, max_lag):
    """Numba-accelerated event counting."""
    if len(timestamps) == 0:
        return 0, 0
    
    good_count = 0
    for i in range(len(timestamps)):
        if timestamps[i] <= T - max_lag:
            good_count += 1
        else:
            break
    
    bad_count = len(timestamps) - good_count
    return good_count, bad_count


@njit(cache=True)
def _numba_symmetrize_arrays(arr1, arr2):
    """Numba-accelerated array symmetrization."""
    n = len(arr1)
    result1 = np.empty(n)
    result2 = np.empty(n)
    
    for i in range(n):
        avg = (arr1[i] + arr2[i]) / 2.0
        result1[i] = avg
        result2[i] = avg
...
@njit(cache=True)
def _numba_compute_interpolation(xe, ye, xs2):
    """Numba-accelerated interpolation computation for _compute_ints_claw."""
    ys2 = np.zeros(len(xs2))
    j = 0
    
    for i in range(1, len(xe)):
        while j < len(xs2) and xs2[j] < xe[i]:
            if xe[i] != xe[i-1]:  # Avoid division by zero
...
@njit(cache=True)
def _numba_compute_integral_arrays(xc, yc):
    """Numba-accelerated integral computation for IG and IG2."""
    n = len(xc)
    
    # Compute IG (integral of claw)
    iyc_IG = np.zeros(n)
    for i in range(1, n):
        dx = xc[i] - xc[i-1]
        iyc_IG[i] = iyc_IG[i-1] + dx * (yc[i-1] + yc[i]) / 2.0
    
    # Compute IG2 (integral of x * claw)  
    iyc_IG2 = np.zeros(n)
    for i in range(1, n):
        dx = xc[i] - xc[i-1]
        dy = yc[i] - yc[i-1]
        
        # Three terms from integration by parts
        term1 = (yc[i-1] + yc[i]) / 2.0 * dx * xc[i-1]
        term2 = dx * dx / 3.0 * dy
        term3 = dx * dx / 2.0 * yc[i-1]
        
        iyc_IG2[i] = iyc_IG2[i-1] + term1 + term2 + term3
    
    return iyc_IG, iyc_IG2


@njit(cache=True)
def _numba_compute_V_matrix(n_index, n_quad, quad_x, claw_values):
    """Numba-accelerated V matrix computation."""
    V = np.zeros((n_index * n_quad, 1))
    
    for index_offset in range(n_index):
        for n in range(n_quad):
            index_i_quad = index_offset * n_quad + n
            # Use pre-computed claw values
            V[index_i_quad, 0] = claw_values[index_offset, n]
    
    return V


@njit(cache=True)  
def _numba_compute_M_matrix_gauss(n_index, n_quad, mark_probabilities, quad_w,
                                 mean_intensity_ratios, G_values, method_is_gauss_minus,
                                 same_indices_matrix):
    """Numba-accelerated M matrix computation for Gaussian quadrature."""
    M = np.zeros((n_index * n_quad, n_index * n_quad))
    
    for index_offset in range(n_index):
        for index1_offset in range(n_index):
            fact = mean_intensity_ratios[index_offset, index1_offset]
            same_indices = same_indices_matrix[index_offset, index1_offset]
            mark_prob_j1_l1 = mark_probabilities[index1_offset]
            
            for n in range(n_quad):
                for n1 in range(n_quad):
                    # Compute x_value inline
                    if n > n1:
                        # Use G_values[index1_offset, n - n1] for difference computation
                        if n - n1 < G_values.shape[1]:
                            x = mark_prob_j1_l1 * quad_w[n1] * G_values[index1_offset, n - n1]
                        else:
                            x = 0.0
                    elif n < n1:
                        # Use G_values[index_offset, n1 - n] for difference computation  
                        if n1 - n < G_values.shape[1]:
                            x = fact * mark_prob_j1_l1 * quad_w[n1] * G_values[index_offset, n1 - n]
                        else:
                            x = 0.0
                    else:  # n == n1
                        if method_is_gauss_minus:
                            x = 0.0
                        else:
                            # Average of both directions
                            x1 = mark_prob_j1_l1 * quad_w[n1] * G_values[index1_offset, 0]
                            x2 = fact * mark_prob_j1_l1 * quad_w[n1] * G_values[index_offset, 0]
                            x = (x1 + x2) / 2.0
                    
                    if method_is_gauss_minus:
                        row = index_offset * n_quad + n
                        col = index1_offset * n_quad + n
                        M[row, col] -= x
                    
                    if same_indices and n == n1:
                        x += 1.0
                    
                    row = index_offset * n_quad + n
                    col = index1_offset * n_quad + n1
                    M[row, col] += x
...
@njit(cache=True)
def _numba_linear_search_closest(x_array, y_array, t, zero_border=True):
    """Numba-accelerated version of _lin0 and _linc methods."""
    n = len(x_array)
    ...
    # Binary search would be faster, but for consistency with original, use linear approach
    ...
    if index == 0:
        ...
        # Choose closest
        if abs(x_array[index] - t) < abs(x_array[index - 1] - t):
            return y_array[index]
        else:
            return y_array[index - 1]
...
def _ppcl_worker(task):
    """Worker for multiprocessing of _PointProcessCondLaw computation.

    Parameters
    ----------
    task : tuple
        (index, i_t, j_t, j_m, lags, mark_min, mark_max, T)
    ...
    tuple
        (index, claw_X, claw_Y)
    """
    (index, i_t, j_t, j_m, lags, mark_min, mark_max, T) = task
    claw_X = np.zeros(len(lags) - 1)
    claw_Y = np.zeros(len(lags) - 1)
    lambda_i = (len(i_t) / T) if T > 0 else 0.0

    PointProcessCondLaw(i_t, j_t, j_m, lags, mark_min, mark_max, T, lambda_i, claw_X, claw_Y)
    return index, claw_X, claw_Y


# noinspection PyPep8Naming
class HawkesConditionalLaw(Base):
    """V3: Same as V2 with multiprocessing for _PointProcessCondLaw in incremental_fit.
    ...
    (See V2 documentation for full details.)
    """
    ...
    def incremental_fit(self, realization, T=None, compute=True):
        """V3: Same as V2 but uses multiprocessing for _PointProcessCondLaw.
        ...
        # V2 OPTIMIZATION: Use numba for mark statistics updates
        for i in range(0, self.n_nodes):
            if len(realization[i][0]) == 0:
                continue
                
            marks = np.asarray(realization[i][1], dtype=np.float64)
            marked_intervals = np.array(self.marked_components[i], dtype=np.float64)
            
            if NUMBA_AVAILABLE:
                interval_counts, new_mark_min, new_mark_max = _numba_compute_mark_stats(
                    marks, marked_intervals, self._mark_min[i], self._mark_max[i])
                
                self._mark_min[i] = new_mark_min
                self._mark_max[i] = new_mark_max
                
                # Update probabilities
                total = 0
                for l in range(len(self.marked_components[i])):
                    self._mark_probabilities_N[i][l] += interval_counts[l]
                    total += self._mark_probabilities_N[i][l]
                
                for l in range(len(self.marked_components[i])):
                    if total > 0:
                        self._mark_probabilities[i][l] = self._mark_probabilities_N[i][l] / total
                    else:
                        self._mark_probabilities[i][l] = 0
            else:
        ...
        # V2 OPTIMIZATION: Vectorized lambda updates
        for i in range(0, self.n_nodes):
            if len(realization[i][0]) <= 0:
                continue
        ...
        # V2 OPTIMIZATION: Use numba for event counting
        for i in range(0, self.n_nodes):
            if NUMBA_AVAILABLE:
                good, bad = _numba_count_valid_events(
                    np.asarray(realization[i][0], dtype=np.float64), T, self._lags[-1])
        ...
        # V3 CHANGE: multiprocessing for _PointProcessCondLaw only
        tasks = []
        ...
        if self.n_threads and self.n_threads > 1:
            with _mp.get_context("spawn").Pool(processes=self.n_threads) as pool:
                for res in pool.imap_unordered(_ppcl_worker, tasks, chunksize=1):
                    results.append(res)
        else:
            for task in tasks:
                results.append(_ppcl_worker(task))

        # Aggregate results back into self._claw
        # Set _claw_X from any result (they are identical across indices)
        if results:
            self._claw_X = results[0][1]
        for index, _claw_X_local, claw_Y in results:
            if self.n_realizations == 0:
        ...
        # V2 OPTIMIZATION: Use numba for symmetrization when possible
        for (i, j) in self.symmetries1d:
            if NUMBA_AVAILABLE:
                # Vectorized symmetrization for mean_intensity
        ...
        for ((i1, j1), (i2, j2)) in self.symmetries2d:
            if NUMBA_AVAILABLE:
                # Use numba for array symmetrization
                arr1 = np.asarray(self._claw1[i1][j1])
                arr2 = np.asarray(self._claw1[i2][j2])
                sym1, sym2 = _numba_symmetrize_arrays(arr1, arr2)
                self._claw1[i1][j1] = sym1
                self._claw1[i2][j2] = sym2
            else:
        ...
        # Ensure no locks are left (locks disallow pickling)
        self._set('_lock', None)
    ...
    def _compute_ints_claw(self):
        """V2: Use original implementation to ensure accuracy.
        
        Computes the claw and its integrals at the difference of
        quadrature points using a linear interpolation
        """
        # Keep original implementation for accuracy
        self._compute_ints_claw_fallback()
    
    def _compute_ints_claw_numba(self):
        """Numba-optimized version of _compute_ints_claw."""
        self._int_claw = [0] * self._n_index
        
        # Pre-compute all quad differences once
        n_quad = len(self._quad_x)
        quad_diffs = np.zeros(n_quad * n_quad)
        idx = 0
        for i in range(n_quad):
            for j in range(n_quad):
                quad_diffs[idx] = self._quad_x[i] - self._quad_x[j]
                idx += 1
        
        xe = self._claw_X
        
        # Process each claw index
        for index in range(self._n_index):
            ye = self._claw[index]
            
            # Build unique sorted x points
            xs2 = np.concatenate([xe, self._quad_x, quad_diffs])
            xs2 = xs2[xs2 >= 0.]
            xs2 = np.unique(xs2)
            xs2.sort()
            ...
            if NUMBA_AVAILABLE:
                ys2 = _numba_compute_interpolation(xe, ye, xs2)
            else:
                # Fallback
                ys2 = np.zeros(len(xs2))
                j = 0
        ...
        # Compute integrals using numba optimization
        self._IG = []
        self._IG2 = []
        ...
        for i in range(self._n_index):
            ...
            if NUMBA_AVAILABLE:
                iyc_IG, iyc_IG2 = _numba_compute_integral_arrays(xc, yc)
            else:
            ...
            self._IG.append((xc, iyc_IG))
            self._IG2.append((xc, iyc_IG2))
    
    def _compute_ints_claw_fallback(self):
    ...
    def _compute_V(self, i, n_index, n_quad, index_first, index_last):
        """V2: Use original implementation to ensure accuracy."""
        # Keep original implementation for accuracy
        return self._compute_V_fallback(i, n_index, n_quad, index_first, index_last)
    
    def _compute_V_numba(self, i, n_index, n_quad, index_first, index_last):
        """Numba-optimized V matrix computation."""
        # Pre-compute all _G values for this i
        claw_values = np.zeros((n_index, n_quad))
        
        for idx, index in enumerate(range(index_first, index_last + 1)):
            (x, j, l) = self._index2ijl[index]
            for n in range(n_quad):
                claw_values[idx, n] = self._G(i, j, l, self._quad_x[n])
        
        # Use numba-optimized matrix computation
        return _numba_compute_V_matrix(n_index, n_quad, self._quad_x, claw_values)
    
    def _compute_V_fallback(self, i, n_index, n_quad, index_first, index_last):
        """Fallback V matrix computation."""
        V = np.zeros((n_index * n_quad, 1))
    ...
    def _compute_M(self, n_index, n_quad, index_first, index_last, method):
        """V2: For now, use original implementation to ensure accuracy."""
        # Revert to original implementation until numba optimization is debugged
        return self._compute_M_fallback(n_index, n_quad, index_first, index_last, method)
    
    def _compute_M_numba_gauss(self, n_index, n_quad, index_first, index_last, method):
        """Numba-optimized M matrix computation for Gaussian quadrature."""
        # Pre-compute all the data needed
        mark_probabilities = np.zeros(n_index)
        mean_intensity_ratios = np.zeros((n_index, n_index))
        same_indices_matrix = np.zeros((n_index, n_index), dtype=np.bool_)
        
        # Pre-compute G values for all quad point differences we might need
        max_diff = n_quad
        G_values = np.zeros((n_index, max_diff))
        
        # Build lookup data
        for idx, index in enumerate(range(index_first, index_last + 1)):
            (x, j, l) = self._index2ijl[index]
            mark_probabilities[idx] = self._mark_probabilities[j][l]
            
            # Pre-compute G values for quad point differences
            for diff in range(max_diff):
                if diff < len(self._quad_x):
                    G_values[idx, diff] = self._G(x, j, l, self._quad_x[diff])
                else:
                    G_values[idx, diff] = 0.0
        
        # Pre-compute intensity ratios and same indices
        for idx in range(n_index):
            for idx1 in range(n_index):
                index = index_first + idx
                index1 = index_first + idx1
                (x, j, l) = self._index2ijl[index]  
                (i1, j1, l1) = self._index2ijl[index1]
                
                mean_intensity_ratios[idx, idx1] = self.mean_intensity[j1] / self.mean_intensity[j]
                same_indices_matrix[idx, idx1] = (l == l1 and j == j1)
        
        # Use numba-optimized computation
        method_is_gauss_minus = (method == 'gauss-')
        return _numba_compute_M_matrix_gauss(
            n_index, n_quad, mark_probabilities, self._quad_w,
            mean_intensity_ratios, G_values, method_is_gauss_minus,
            same_indices_matrix
        )
    
    def _compute_M_fallback(self, n_index, n_quad, index_first, index_last, method):
        """Fallback M matrix computation."""
        M = np.zeros((n_index * n_quad, n_index * n_quad))
        for index in range(index_first, index_last + 1):
            (x, j, l) = self._index2ijl[index]
